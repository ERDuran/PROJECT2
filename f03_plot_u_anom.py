...
for f in file_number:
    CT = nc.Dataset(data_path + input_folder_path[0] + var[2] + '_' + str(f) + '-' + str(f+3) + '.nc', 'r')
    PI = nc.Dataset(data_path + input_folder_path[2] + var[2] + '_' + str(f) + '-' + str(f+3) + '.nc', 'r')

    time_now = int(round(CT.variables['time'][0]/365))

    time.append(time_now)
    
    CT_temp[str(time_now)] = CT.variables[var[2]][0,0,:,:]
    PI_temp[str(time_now)] = PI.variables[var[2]][0,0,:,:]

    PI_temp_anom[str(time_now)] = PI_temp[str(time_now)] - CT_temp[str(time_now)]
    print(time_now)


lat = CT.variables['yu_ocean'][:]
lon1 = CT.variables['xu_ocean'][:]
lon = lon1 + 360
...
for t in time:
    ...
    cbar.set_label(r'U sfc $m/s$') # units label on colourbar
    ...
    step = 0.1
    # levels to show on colourbar
    contf_lvls = np.arange(-1.0,1.0+1e-08,step)
    ...
    #cbar_axe = fig.add_axes(cbar_pos)
    cbar = plt.colorbar(contf, orientation='horizontal', drawedges=True)
    cbar.set_label(r'U sfc anomaly $m/s$') # units label on colourbar
    cbar.dividers.set_linewidth(0.2)
    cbar.outline.set_linewidth(0.5)
    cbar.set_ticks(contf_lvls[np.arange(0,np.size(contf_lvls),2)])
    cbar.ax.tick_params(width=0.2, length= 2)
    ...
    plt.suptitle(r"KDS75 year to year U at the surface in PI which starts at year 69.", 
                 y=0.97, fontsize=20)
