...
for f in file_number:
    CT = nc.Dataset(data_path + input_folder_path[0] + var[1] + '_' + str(f) + '-' + str(f+3) + '.nc', 'r')
    PI = nc.Dataset(data_path + input_folder_path[2] + var[1] + '_' + str(f) + '-' + str(f+3) + '.nc', 'r')

    time_now = int(round(CT.variables['time'][0]/365))

    time.append(time_now)
    
    CT_temp[str(time_now)] = CT.variables[var[1]][0,0,:,:]
    PI_temp[str(time_now)] = PI.variables[var[1]][0,0,:,:]
...
for t in time:
    ...
    cbar.set_label(r'SSS $psu$') # units label on colourbar
    ...
    #cbar_axe = fig.add_axes(cbar_pos)
    cbar = plt.colorbar(contf, orientation='horizontal', drawedges=True)
    cbar.set_label(r'SSS anomaly $psu$') # units label on colourbar
    cbar.dividers.set_linewidth(0.2)
    cbar.outline.set_linewidth(0.5)
    cbar.set_ticks(contf_lvls[np.arange(0,np.size(contf_lvls),2)])
    cbar.ax.tick_params(width=0.2, length= 2)
    ...
    plt.suptitle(r"KDS75 year to year SSS in PI which starts at year 69.", 
                 y=0.97, fontsize=20)
