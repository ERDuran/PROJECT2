...
for d, d_out in zip(DATA, DATA_out):
    for m in MMM:
        ...
        nc_fid = nc.Dataset(data_path + 'KDS75' + d + '/temp_'
                            + m + '_0.nc', 'r')
        # same for temperature (1,2,3)
        out[d_out + m] = nc_fid.variables['temp'][0,z_idx,:,:]
...
for m in MMM:
    for d, d_out in zip(DATA, DATA_out):
        ...
        if d_out is 'CT':
            ...
            contf_lvls = np.arange(0,28+1e-08,step)
        ...
        if s is 10:
            ...
            cbar.set_label(r'Absolute ${\theta}$ $(^{\circ}C)$') # units label on colourbar
            ...
            cbar = plt.colorbar(contf, cax=cbar_axe, 
                                orientation='vertical', drawedges=True)
            cbar.set_label(r'${\theta}$ anomaly $(^{\circ}C)$') # units label on colourbar
            cbar.dividers.set_linewidth(0.2)
            cbar.outline.set_linewidth(0.5)
            cbar.set_ticks(contf_lvls[np.arange(0,np.size(contf_lvls),2)])
            cbar.ax.tick_params(width=0.2, length= 2)
...
plt.suptitle(r"Sea Surface Temperature", y=0.97, fontsize=20)
